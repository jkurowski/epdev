...
@section('content')
    @if (Route::is('admin.developro.investment.article.edit'))
        <form method="POST" action="{{ route('admin.developro.investment.article.update', [$investment, $entry]) }}"
            enctype="multipart/form-data">
            @method('PUT')
        @else
            <form method="POST" action="{{ route('admin.developro.investment.article.store', $investment) }}"
                enctype="multipart/form-data">
    @endif
    @csrf
    <div class="container">
            ...
            <div class="row">
                <div class="col-12 pl-0">
                    <h4 class="page-title"><i class="fe-home"></i><a
                            ...
                            href="{{ route('admin.developro.investment.article.index', $investment) }}">{{ $investment->name }}:
                            Aktualności</a><span class="d-inline-flex me-2 ms-2">/</span>{{ $cardTitle }}</h4>
                </div>
            </div>
        </div>
        ...
        <div class="card mt-3">
            ...
            <div class="card-body control-col12">
                ...
                @if ($entry->file)
                    <div class="row w-100 form-group">
                        <label for="file" class="col-12 col-form-label control-label pb-2">
                            <div class="text-right">Wybrane zdjęcie</div>
                        </label>
                        <div class=" col-12 control-input position-relative " >
                            <img src="{{ asset('investment/articles/thumbs/' . $entry->file) }}" class="object-fit-contain" style="height: 200px; width: auto;"
                                alt="Podgląd zdjęcia">
                        </div>
                    </div>
                @endif
